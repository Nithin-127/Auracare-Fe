        ...
        <nav>
          <ul className="nav-menu">
            ...
            {userData?.role !== 'receiver' && (
              <li><Link to="/donor" className={`nav-link ${isActive('/donor')}`}>Donor</Link></li>
            )}
            {userData?.role !== 'donor' && (
              <li><Link to="/receiver" className={`nav-link ${isActive('/receiver')}`}>Receiver</Link></li>
            )}
            {userData?.role === 'donor' && (
              <li><Link to="/receivers-list" className={`nav-link ${isActive('/receivers-list')}`}>Receivers List</Link></li>
            )}
            <li><Link to="/aura" className={`nav-link ${isActive('/aura')}`}>Donors List</Link></li>
            <li><Link to="/contact" className={`nav-link ${isActive('/contact')}`}>Contact</Link></li>
            {isAuthorized && (
              userData?.isPremium ? (
                <li><Link to="/book-doctor" className={`nav-link ${isActive('/book-doctor')}`} style={{color: '#d97706', fontWeight: 'bold'}}>Consultation</Link></li>
              ) : (
                <li><Link to="/premium" className={`nav-link ${isActive('/premium')}`} style={{color: '#d97706', fontWeight: 'bold'}}>Go Premium 🌟</Link></li>
              )
            )}
